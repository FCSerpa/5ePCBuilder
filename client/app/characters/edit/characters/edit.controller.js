...
angular.module('5ePcApp')
  .controller('CharactersEditCtrl', function ($scope, $stateParams, User, Character, Auth, $location) {
    ...
    $scope.isPlayer = function() {
    	if ($scope.getCurrentUser()._id === $scope.character.user_id) {
    		return true;
    	} else {
    		return false;
    	}
    ...
    $scope.clericChoiceFunc = function() {
        if ($scope.clericChoice === 'light_crossbow') {
            $scope.ifAnySimpleWeapon = false;
            $scope.character.classEquipment.weapon[1] = 'light_crossbow';
    ...
    $scope.fighterChoice2Func = function() {
        if ($scope.fighterChoice2 === 'shield') {
            $scope.character.classEquipment.armor[1] = 'shield';
            $scope.character.classEquipment.weapon[1] = '';
            $scope.isOneWeapon = true;
            $scope.isTwoWeapons = false;
        } else if ($scope.fighterChoice2 === '2 weapons') {
            $scope.character.classEquipment.armor[2] = '';
            $scope.isOneWeapon = true;
    ...
    $scope.fighterChoice3Func = function() {
        if ($scope.fighterChoice3 === 'crossbow') {
            $scope.character.classEquipment.weapon[2] = 'light_crossbow';
            $scope.character.classEquipment.weapon[6] = '20 bolts';
        } else if ($scope.fighterChoice3 === 'handaxes') {
            $scope.character.classEquipment.weapon[2] = 'handaxe';
    ...
    $scope.getLanguages = function(){
        if ($scope.isDwarf()) {
            //Common, Dwarvish
            $scope.character.proficiencies.languages[0] = 'Common';
            $scope.character.proficiencies.languages[1] = 'Dwarvish';
        } else if ($scope.isElf()) {
            //Common, Elvish
            $scope.character.proficiencies.languages[0] = 'Common';
            $scope.character.proficiencies.languages[1] = 'Elvish';
        } else if ($scope.isHalfling()) {
            //Common, Halfling
            $scope.character.proficiencies.languages[0] = 'Common';
            $scope.character.proficiencies.languages[1] = 'Halfling';
        } else if ($scope.isHuman()) {
            //Common, choose one
            $scope.character.proficiencies.languages[0] = 'Common';
        }
        ...
        if (!$scope.isHighElf()) {
            $scope.character.proficiencies.languages[2] = '';
        }

        if (!$scope.isAcolyte() && !$scope.isSage()) {
            $scope.character.proficiencies.languages[3] = '';
            $scope.character.proficiencies.languages[4] = '';
        }
    };
    ...
    $scope.resetToolProficiencies = function() {
        if (!$scope.isDwarf()){
            ...
            $scope.character.proficiencies.tools[0] = '';
        }
        if ($scope.isRogue()) {
            //'thieves’ tools'
            $scope.character.proficiencies.tools[1] = 'thieves’ tools';
        } else {
            $scope.character.proficiencies.tools[1] = '';
        }
        if ($scope.isCriminal()) {
            $scope.character.proficiencies.tools[2] = 'thieves’ tools';
        } else if ($scope.isFolkHero() || $scope.isSoldier()) {
            $scope.character.proficiencies.tools[2] = 'vehicles (land)';
        } else {
            ...
            $scope.character.proficiencies.tools[2] = '';
            $scope.character.proficiencies.tools[3] = '';
        }
    };
    ...
    $scope.getArmorProficiencies = function() {
        if ($scope.isLifeDomain() || $scope.isFighter()){
            $scope.character.proficiencies.armorType = 2;
        }else if (($scope.isCleric() && !$scope.isLifeDomain()) || ($scope.isMountainDwarf() && $scope.isWizard()) || ($scope.isMountainDwarf() && $scope.isRogue()))  {
            $scope.character.proficiencies.armorType = 1;
        } else if ($scope.isWizard()) {
            $scope.character.proficiencies.armorType = -1;
        } else if ($scope.isRogue()) {
            $scope.character.proficiencies.armorType = 0;
        } 
        if ($scope.isCleric() || $scope.isFighter()) {
            $scope.character.proficiencies.armorName = 'shields';
        } else {
            $scope.character.proficiencies.armorName = '';
        }
    };
    ...
    function weaponProficiencies(){
        if ($scope.isCleric()){
            $scope.character.proficiencies.weaponType = 0;
        } else if ($scope.isFighter()){
            $scope.character.proficiencies.weaponType = 1;
        } else if ($scope.isRogue()){
            $scope.character.proficiencies.weaponType = 0;
        } else if ($scope.isWizard()){
            $scope.character.proficiencies.weaponType = -1;
        ...
        if ($scope.isWizard() && $scope.isElf()){
            $scope.character.proficiencies.weaponName =['light_crossbow', 'dagger', 'dart', 'sling', 'quarterstaff', 'longsword', 'shortsword', 'longbow', 'shortbow'];
        } else if ($scope.isWizard() && $scope.isDwarf()){
            $scope.character.proficiencies.weaponName =['light_crossbow', 'dagger', 'dart', 'sling', 'quarterstaff', 'battleaxe', 'handaxe', 'light_hammer', 'warhammer'];
        } else if ($scope.isWizard() && (!$scope.isDwarf() || !$scope.isElf())){
            $scope.character.proficiencies.weaponName =['light_crossbow', 'dagger', 'dart', 'sling', 'quarterstaff'];
        } else if ($scope.isRogue() && $scope.isElf()){
            $scope.character.proficiencies.weaponName = ['hand_crossbow', 'shortsword', 'longsword', 'rapier', 'longbow'];
        } else if ($scope.isRogue() && $scope.isDwarf()){
            $scope.character.proficiencies.weaponName = ['hand_crossbow', 'shortsword', 'longsword', 'rapier', 'battleaxe', 'handaxe', 'light_hammer', 'warhammer'];
        } else if ($scope.isRogue() && (!$scope.isDwarf() || !$scope.isElf())){
            $scope.character.proficiencies.weaponName = ['hand_crossbow', 'shortsword', 'longsword', 'rapier'];
        } else if ($scope.isDwarf() && $scope.isCleric()){
            $scope.character.proficiencies.weaponName = ['battleaxe', 'handaxe', 'light_hammer', 'warhammer'];
        } else if ($scope.isElf() && $scope.isCleric()){
            $scope.character.proficiencies.weaponName = ['longsword', 'shortsword', 'longbow', 'shortbow'];
        } else {
             $scope.character.proficiencies.weaponName = [];
        }
    }
 ...
 $scope.updateCharacter = function() {
        $scope.character.image = $scope.character.image || '../assets/images/profilegandalf.jpg';
        if (typeof $scope.character.level === 'string'){
            $scope.character.level = parseInt($scope.character.level);
        }
        if (typeof $scope.character.abilities.str === 'string'){
            $scope.character.abilities.str = parseInt($scope.character.abilities.str);
        }
        if (typeof $scope.character.abilities.dex === 'string'){
            $scope.character.abilities.dex = parseInt($scope.character.abilities.dex);
        }
        if (typeof $scope.character.abilities.con === 'string'){
            $scope.character.abilities.con = parseInt($scope.character.abilities.con);
        }
        if (typeof $scope.character.abilities.intel === 'string'){
            $scope.character.abilities.intel = parseInt($scope.character.abilities.intel);
        }
        if (typeof $scope.character.abilities.wis === 'string'){
            $scope.character.abilities.wis = parseInt($scope.character.abilities.wis);
        }
        if (typeof $scope.character.abilities.cha === 'string'){
            $scope.character.abilities.cha = parseInt($scope.character.abilities.cha);
        }
